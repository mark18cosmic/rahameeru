...
import { Search, Flame, Leaf, Star, X, UtensilsCrossed } from "lucide-react";
import type { MenuSection } from "@/app/lib/types";
import { cx } from "@/app/lib/utils";

/** Dietary filters, matched against an item's `tags`. */
const FILTERS = [
  { key: "Vegetarian", label: "Vegetarian", icon: Leaf, match: ["Vegetarian", "Vegan"] },
  ...
  return (
    <section id="menu" className="scroll-mt-24">
      ...
      {!filtering && sections.length > 1 && (
        <div className="scrollbar-hide sticky top-16 z-20 -mx-4 mt-4 flex gap-2 overflow-x-auto border-b border-ink-100 bg-[var(--bg)]/90 px-4 py-2 backdrop-blur dark:border-ink-800 md:mx-0 md:px-0">
          {sections.map((s) => (
            <button
              key={s.name}
      ...
      {/* Sections */}
      <div ref={listRef} className="mt-4 space-y-8">
        {visible.map((section) => (
          <div key={section.name} data-section={section.name} className="scroll-mt-28">
